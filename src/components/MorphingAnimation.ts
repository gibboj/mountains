...
export class MorphingAnimation {
  ...
  constructor(
    paths: Array<string>,
    keyFrames: Array<number>,
    options: MorphOptions = defaultOptions
  ) {
    ...
    if (keyFrames.length !== paths.length - 1) {
      throw new Error(
        `there should be one less keyFrame than paths length. Paths: 
        ${paths.length} | KeyFrame: ${keyFrames.length}`
      );
    }
  ...
  getPath(time: number) {
    ...
    if (interpolator) {
      ...
      const percentage = Math.min(
        1,
        (time - startTime) / (this.keyFrames[index] - startTime)
      );
